        ...
        <div className="min-w-80 bg-background-secondary text-text-primary px-4 py-2 rounded-md flex flex-col cursor-pointer">
          ...
          <div>
            <div className="flex items-center">
              {dataInputType === "boolean" ? (
                <div className="w-64 bg-background-primary rounded flex h-10 items-center">
                  <label
                    htmlFor={`check-input-${data._id}`}
                    className={`bg-background-secondary rounded h-1/4 flex w-14 mx-2 relative items-center ${dataInput ? "justify-end" : "justify-start"}`}
                  >
                    <div
                      className={`w-5 ${dataInput ? "bg-green-500" : "bg-gray-400"} h-[200%] absolute rounded-full`}
                    ></div>
                    <input
                      type="checkbox"
                ...
                <input
                  ...
                  placeholder={
                    data.signals.filter(
                      (item) => item.signal === dataInputType && item,
                    )[0]?.placeholder ?? ""
                  }
                  type="text"
              ...
              >
                {data.signals.map((signal, i) => (
                  <option key={i} value={signal.signal}>
                    {signal.name}
                  </option>
                ))}
              </select>

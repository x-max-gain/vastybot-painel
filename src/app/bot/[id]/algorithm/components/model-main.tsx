...
export default function CreateBotAlgorithmModelMain({
  ...
  setElementSelect,
}: CreateBotAlgorithmModelMainType) {
      ...
      <div className="flex items-start mb-4">
        ...
        <div className="col-span-4 flex">
          {data?.options?.left && data?.options?.left.type === "func" && (
            <CreateBotAlgorithmModelFunc
              informationsActiveSelected={informationsActiveSelected}
              startOpen={true}
              data={data?.options?.left.params}
            />
          )}
          ...
          {data?.options?.left &&
            data?.options?.left.type !== "func" &&
            data?.options?.left.type !== "data" && (
        ...
        <div className="col-span-2 flex items-center justify-center mx-2">
          <select
            name=""
            id=""
            className="rounded-md cursor-pointer w-full py-2 px-4 text-center min-w-36"
          >
        ...
        <div className="col-span-4 flex">
          {data?.options?.right && data?.options?.right.type === "func" && (
            <CreateBotAlgorithmModelFunc
              informationsActiveSelected={informationsActiveSelected}
              data={data?.options?.right.params}
              startOpen={true}
            />
          )}
          ...
          {data?.options?.right &&
            data?.options?.right.type! !== "func" &&
            data?.options?.right.type !== "data" && (
              <CreateBotAlgorithmModelMain
                setElementSelect={selectOptionRight}
                informationsActiveSelected={informationsActiveSelected}
                data={data?.options?.right}
              />
            )}
          {!data?.options?.right && (
            <CreateBotAlgorithmSelect
        ...
        <div className="flex">
          ...
          <div className="col-span-10 w-full">
            {data?.options?.continue ? (
              <CreateBotAlgorithmModelMain
                setElementSelect={selectOptionContinue}
                informationsActiveSelected={informationsActiveSelected}
                data={data?.options?.continue}
              />
            ) : (
              <CreateBotAlgorithmSelect
                title="Selecione um opção"

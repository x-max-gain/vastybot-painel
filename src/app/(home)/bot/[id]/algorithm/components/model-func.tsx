...
export default function CreateBotAlgorithmModelFunc({
  data,
  informationsActiveSelected,
  startOpen = false,
}: CreateBotAlgorithmModelFuncType) {
  ...
  return (
    <div className="select-bot relative">
      {!selectOption && (
        <div
          onClick={() => setOpen(!open)}
          className="min-w-80 bg-background-secondary text-text-primary px-4 py-2 rounded-md flex items-center cursor-pointer"
        >
          {!open ? (
      ...
      {selectOption && (
        <div className="min-w-80 bg-background-secondary text-text-primary px-4 py-2 rounded-md flex flex-col relative">
          <div className="top-[-10px] left-[-10px] p-1 bg-background-danger rounded-full text-text-ligth cursor-pointer absolute">
            <X size={12} />
          </div>
          <div className="flex justify-between py-2 mb-2 border-b border-gray-300">
            ...
            <div className="flex items-center">
              <div
                ...
                className="p-1 bg-background-warnning rounded-full cursor-pointer"
              >
            ...
            {selectOption.params.map((param, index) => (
              <li key={index} className="flex items-center">
                ...
                <div className="flex w-full ml-2 mb-1">
                  ...
                  {param.input.type === "price" && (
                    <div className="w-full flex items-center bg-background-primary h-10 px-2">
                      <p className="mr-2">
                        {informationsActiveSelected.selectedActive.coin.signal}
                      </p>
                      <input
                        inputMode="numeric"
                        name={param.name}
                  ...
                  {param.input.type === "boolean" && (
                    <div className="w-full bg-background-primary rounded flex h-10 items-center">
                      <label
                        htmlFor={`check-input-${param.name}`}
                        className={`bg-background-secondary rounded h-1/4 flex w-2/12 mx-2 relative items-center ${dataFunction[param.name] ? "justify-end" : "justify-start"}`}
                      >
                        <div
                          className={`w-5 ${dataFunction[param.name] ? "bg-green-500" : "bg-gray-400"} h-[200%] absolute rounded-full`}
                        ></div>
                        <input
                          type="checkbox"
                          onChange={(event: ChangeEvent<HTMLInputElement>) =>
                ...
                >
                  ...
                  <BotInfo
                    setOpen={setSelectInfo}
                    info={param.info}
                    open={selectInfo === `${index}`}
                  />
                </div>
              </li>
            ))}
          ...
          {!minimize && selectOption.params.length > 0 && (
            <div className="flex justify-end mt-2">
              <button className="bg-background-main font-bold rounded text-white py-2 w-full">
                Salvar
              </button>
            </div>
          )}
      ...
      {open && (
        <div className="z-20 max-h-96 overflow-y-auto overflow-visible select-bot-list bg-background-primary border border-gray-500 w-full rounded-md">
          <ul>
            {data.map((item, index) => (
              <>
                {item.type == "function" ? (
                  ...
                  <li key={index}>
                    ...
                    >
                      <p onClick={(event) => clickCategory(event, item._id)}>
                        {item.title}
                      </p>
                      <ChevronDown
                        onClick={(event) => clickCategory(event, item._id)}
                        className="size-6 text-text-primary2"
                      />
                    </div>
                    {opensCategories.includes(item._id) && (
                      <ul>
                        {item.functions.map((func: any, index2: number) => (
                          ...
                          >
                            <div
                              onClick={() => {
                                selectOptionFunction(item.functions[index2]);
                                setOpen(false);
                              }}
                              className="w-full flex items-center px-4 py-2 hover:bg-background-secondary"
                            >
                              <Code className="mr-2 size-4 text-text-primary2" />
                              {func.title}
                            </div>
                            <div
